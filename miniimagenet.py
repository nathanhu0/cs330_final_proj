...
def load_image(file_path):
    """Loads and transforms an minimagenet image.
    ...
    x = torch.tensor(x, dtype=torch.float32).reshape([3, 84, 84])
    x = x / 255.0
    return x



class MiniImageNetDataset(dataset.Dataset):
    """MiniImageNet dataset for meta-learning.
...
class MiniImageNetSampler(sampler.Sampler):
...
def get_miniimagenet_dataloader(
        split,
        batch_size,
        num_way,
        num_support,
    ...
    """Returns a dataloader.DataLoader for MiniImageNet.
    ...
    return dataloader.DataLoader(
        dataset=MiniImageNetDataset(num_support, num_query),
        batch_size=batch_size,
        sampler=MiniImageNetSampler(split_idxs, num_way, num_tasks_per_epoch),
        num_workers=2,
        collate_fn=identity,
