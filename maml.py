...
class MAML:
    ...
    def _forward(self, images, parameters):
        ...
        for i in range(NUM_CONV_LAYERS):
            x = F.conv2d(
                input=x,
                weight=parameters[f'conv{i}']*self._mask[f'conv{i}'],
        ...
        return F.linear(
            input=x,
            weight=parameters[f'w{NUM_CONV_LAYERS}']*self._mask[f'w{NUM_CONV_LAYERS}'],
            bias=parameters[f'b{NUM_CONV_LAYERS}']*self._mask[f'b{NUM_CONV_LAYERS}']
        )
